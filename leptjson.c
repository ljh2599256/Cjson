...
#ifndef LEPT_PARSE_STATCK_INIT_SIZE
#define LEPT_PARSE_STATCK_INIT_SIZE 256
#endif
...
static void* lept_context_push(lept_context* c, size_t size) {
    ...
    if (c->size - c->top <= size) {
        if (c->size == 0)
            c->size = LEPT_PARSE_STATCK_INIT_SIZE;
...
static int lept_parse_value(lept_context* c, lept_value* v) {
    switch (*c->json) {
        case 'n': lept_parse_literal(c, v, "null", LEPT_NULL); break;
        case 't': lept_parse_literal(c, v, "true", LEPT_TRUE); break;
        case 'f': lept_parse_literal(c, v, "false", LEPT_FALSE); break;
        case '\0': return LEPT_PARSE_EXPECT_VALUE;
        case '"': lept_parse_string(c, v); break;
